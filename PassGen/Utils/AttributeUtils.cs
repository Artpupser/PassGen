...
public static class Tools {
	...
	public static string Hash256(object content) {
		return string.Join(string.Empty,
			SHA256.HashData(Encoding.UTF8.GetBytes(content.ToString() ?? string.Empty)).Select(b => b.ToString("x2")));
	}

	public static string HashMD5(object content) {
		return string.Join(string.Empty,
			MD5.HashData(Encoding.UTF8.GetBytes(content.ToString() ?? string.Empty)).Select(b => b.ToString("x2")));
	}
}
