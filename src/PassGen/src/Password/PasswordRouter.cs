...
public sealed class PasswordRouter {
   ...
   public Option<IEnumerable<PasswordGeneratorInfoAttribute>> GetAttributes() {
      return _types.Length == 0 ? Option<IEnumerable<PasswordGeneratorInfoAttribute>>.Fail() : Option<IEnumerable<PasswordGeneratorInfoAttribute>>.Ok(_types.Select(x => x.GetCustomAttribute<PasswordGeneratorInfoAttribute>())!);
   }
   
   public Option ChangeModel(string name) {
      return ChangeModel(() => _types.FirstOrDefault(x => x.GetCustomAttribute<PasswordGeneratorInfoAttribute>(false)!.Name == name));
   }
